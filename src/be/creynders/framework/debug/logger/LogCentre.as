package be.creynders.framework.debug.logger {
	...
	//--------------------------------------------------------------------------
		...
		/* INTERFACE be.creynders.framework.debug.logger.ILogCentre */
		...
		override public function log(level:LogLevel, target:Object, object:*):void{
			if ( _updateDelay <= 0 ) {
				dispatch( level, target, object );
			}else {
				_buffer.push( { level : level, target : target, object : object } );
			}

		}
		
		private function dispatch( level : LogLevel, target : Object, object : * ) : void {
			for each( var logger : ILogger in _loggers ) logger.log( level, target, object );
		}
		...
		private function flushBuffer() : void {
			...
			for( i = 0 ; i < n ; i++ ) {
				...
				dispatch( message.level, message.target, message.object );
			}
		}
